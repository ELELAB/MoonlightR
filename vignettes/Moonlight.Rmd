...
dataFilt <- getDataTCGA(cancerType = cancerType, 
                          dataType = "Gene expression",
                          directory = "data",
                          nSample = 10,
                          subtype = subtype,
...
# Prepare mutation's data for subtypes
...
load("~/Dropbox/IB2_postdoc/Github/TCGA/AnalysisFilt/BRCA_IlluminaHiSeq_RNASeqV2_Filt.rda")

res.mutation <- NULL
  for (i in 1: length(CancerSubtypes)){

    dataSmTP <- colnames(dataFilt)

  subtype <- CancerSubtypes[i]
  dataSmSubt <- dataSubt_Cancer[dataSubt_Cancer$PAM50.mRNA %in% subtype,]


dataSubtC <- dataSmTP[substr(dataSmTP,1,12) %in% dataSmSubt$patient]
dataSmTP <- dataSubtC
...
names(res.mutation) <-names(listMoonlight)
...
names(tmp) <- names(listMoonlight)
 ...
 plotCircos(listMoonlight=listMoonlight, additionalFilename="brca_subtypes", intensityColDual=0.2,fontSize = 2)
...
In this section we showed downstream analysis with replication of Moonlight in TCGA's data within comparison of different molecular subtypes and normal samples.
You can use the function `GDCquery_Maf` from TCGAbiolinks to retrieve this information.
