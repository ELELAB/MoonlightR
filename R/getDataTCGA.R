#' getDataTCGA
#'
#' This function retrieves and prepares TCGA data (hg19)
...
#' @param dataType is dataType such as gene expression, cnv, DNA methylation etc. 
#' Options: "Methylation" (27k) and "Gene expression"
  ...
  if(dataType == "Gene expression"){
    ...
    if(!file.exists(FileNameData)){
      ...
      if (is.numeric(stage)==TRUE){
        dataClin <- GDCquery_clinic(project = CancerProject, type = "clinical")
      ...
      message(paste0(CancerProject," ... with ", length(dataSmTP), " Primary solid Tumor, ", length(dataSmNT), " Solid Tissue Normal"))
      ...
      RSEobject <- get(load(FileNameData))
    }
    ...
    if(!file.exists(FileNameData)){
      ...
      message(CancerProject," ... with ", length(dataSmTP), " Primary solid Tumor, ", length(dataSmNT), " Solid Tissue Normal")
